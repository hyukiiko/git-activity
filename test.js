function testHTML() {
  ...
  function assert(condition, message) {
    if (condition) {
      ...
      throw new Error('DEPLOYMENT CRASHED: index.html not found or invalid');
    }
  }
...
<ul id="team-list">
  ...
  assert(htmlContent.includes('index.html') || htmlContent.trim().length > 0, 'index.html file must exist');
...
<ul id="team-list">
  ...
  assert(doc.querySelector('h1').textContent === 'Hello GitHub Actions!', 'H1 should contain "Hello GitHub Actions!"');
  assert(doc.querySelector('#team-list') !== null, 'Team list should exist');
  assert(typeof window !== 'undefined' && window.indexHtmlExists === true, 'index.html file must exist');
